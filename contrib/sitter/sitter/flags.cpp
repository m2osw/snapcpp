 ...
 * one conf_file.
 *
 * \return The path where to create flag files.
 */
std::string get_path_to_flag_files()
{
    ...
    if(g_path_to_flag_files.empty())
    {
        // get the path (once)
        //
        advgetopt::conf_file_setup setup("/etc/sitter/flags.conf");
        advgetopt::conf_file::pointer_t server_config(advgetopt::conf_file::get_conf_file(setup));
        if(server_config->has_parameter("path"))
        {
            g_path_to_flag_files = server_config->get_parameter("path");
        }
        else
        {
            g_path_to_flag_files = "/var/lib/sitter/flags";
        }
    }
 ...
 * \endcode
 *
 * \return The filename to use for this flag.
 */
std::string const & flag::get_filename() const
{
    if(f_filename.empty())
    {
        f_filename = get_path_to_flag_files() + "/" + f_unit + "_" + f_section + "_" + f_name + ".flag";
    }
    return f_filename;
}
...
bool flag::save()
{
    ...
    if(f_state == state_t::STATE_UP)
    {
        ...
        file->set_parameter(std::string(), "version",     SITTER_VERSION_STRING);
...
void flag::valid_name(std::string & name)
{
    if(name.empty())
    {
        throw flags_exception_invalid_name("unit, section, name, tags cannot be empty");
    }
    ...
    for(std::string::size_type idx(0); idx < max; ++idx)
    {
        ...
        case '-':
            if(first)
            {
                throw flags_exception_invalid_name("unit, section, name, tags cannot start with a dash (-)");
            }
            if(last_char == '-')
            {
                throw flags_exception_invalid_name("unit, section, name, tags cannot have two dashes (--) in a row");
            }
            break;
        ...
        case '9':
            if(first)
            {
                throw flags_exception_invalid_name("unit, section, name, tags cannot start with a digit (-)");
            }
            break;
        ...
        default:
            ...
            {
                throw flags_exception_invalid_name("name cannot include characters other than a-z, 0-9, and dashes (-)");
            }
            break;
    ...
    if(last_char == '-')
    {
        throw flags_exception_invalid_name("unit, section, name, tags cannot end with a dash (-)");
    }
}
