...
{
    SNAP_LOG_DEBUG
        << "watchscripts::on_process_watch(): processing"
        << SNAP_LOG_SEND;
...
void watchscripts::process_script(int index, std::string script_filename)
{
...
QString watchscripts::generate_header(QString const & type)
{
    ...
    {
        header += "Hostname: ";
        header += QString::fromUtf8(hostname.contents().c_str()).trimmed();
        header += "\n";
    }
    ...
    {
        header += "IP-Address: ";
        header += QString::fromUtf8(my_ip.c_str());
        header += "\n";
    }
    else
    {
...
//bool watchscripts::output_available(cppprocess::io * io, cppprocess::done_reason_t reason)
//{
...
//bool watchscripts::error_available(cppprocess::io * io, cppprocess::done_reason_t reason)
//{
