...
bool watchdog_process_t::is_process_expected_to_run()
{
    ...
    if(is_backend())
    {
        ...
        {
            QString const expected_backends(snap_server_conf->get_parameter("backends").c_str());
 ...
 */
void load_xml(int index, std::string processes_filename)
{
...
void load_processes(QString processes_path)
{
    ...
    script_filenames.read_path<
        ...
        , snapdev::glob_to_list_flag_t::GLOB_FLAG_IGNORE_ERRORS>(std::string(processes_path.toUtf8().data()) + "/*.xml");
    snapdev::enumerate(script_filenames, std::bind(&load_xml, std::placeholders::_1, std::placeholders::_2));
}
...
char const * get_name(name_t name)
{
    ...
    default:
        // invalid index
        throw snap_logic_error("Invalid SNAP_NAME_WATCHDOG_PROCESSES_...");
