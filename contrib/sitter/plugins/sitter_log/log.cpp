...
char const * get_name(name_t name)
{
    ...
    default:
        // invalid index
        throw snap_logic_error("Invalid SNAP_NAME_WATCHDOG_LOG_...");
    ...
    for(size_t idx(0); idx < max_logs; ++idx)
    {
        ...
        for(auto const & p : patterns)
        {
            ...
            log_filenames.read_path<
                ...
                , snapdev::glob_to_list_flag_t::GLOB_FLAG_IGNORE_ERRORS>(std::string(path.toUtf8().data()) + "/" + p.toUtf8().data());
            snapdev::enumerate(
                  log_filenames
                , std::bind(&log::check_log, this, std::placeholders::_1, std::placeholders::_2, l, e));
        }
        if(!f_found)
        {
...
void log::check_log(int index, std::string filename, watchdog_log_t const & l, QDomElement e)
{
    ...
    if(stat(filename.c_str(), &st) == 0)
    {
        ...
        log_tag.setAttribute("filename", QString::fromUtf8(filename.c_str()));
