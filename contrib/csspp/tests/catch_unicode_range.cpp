...
TEST_CASE("Unicode range: start/end equal", "[unicode-range] [code-point]")
{
    for(int i(0); i < 1000; ++i)
    {
        ...
        REQUIRE(range.to_string() == ss.str());
    }
...
TEST_CASE("Unicode range: start/end differ", "[unicode-range] [range]")
{
    ...
    for(int i(0); i < 1000; ++i)
    {
        ...
        REQUIRE(range.get_range() == our_range.get_range());

        REQUIRE(range.get_start() == unicode_start);
        REQUIRE(range.get_end() == unicode_end);
        ...
        REQUIRE(new_range.get_range() == our_range.get_range());

        REQUIRE(new_range.get_start() == unicode_start);
        REQUIRE(new_range.get_end() == unicode_end);

        ss.str("");
        ss << std::hex << unicode_start << "-" << unicode_end;

        REQUIRE(new_range.to_string() == ss.str());
    }
