    ...
    if( f_website.isEmpty() )
    {
        ...
        try
        {
            ...
            for(;;)
            {
                ...
                // got some websites
                //
                libdbproxy::QCassandraRows const rows(f_sites_table->rows());
                for(libdbproxy::QCassandraRows::const_iterator it(rows.begin());
                                                                it != rows.end();
                                                                ++it)
                {
...
bool snap_backend::process_timeout()
{
    ...
    if( f_website.isEmpty() )
    {
        ...
        try
        {
            ...
            //
            libdbproxy::row::pointer_t row(f_backend_table->row(f_action));
