// Snap Websites Server -- test shutdown() on socket (server side)
...
//
// This test is used to prove that calling shutdown() on a socket kills
// the use of that socket for anyone who has a copy of the file descriptor:
// the parent and the child.
//
// The test makes use of the test_shutdown_client.cpp as well. To run the
// test, start the test_shutdown_server binary first, then start the
// test_shutdown_client. Then both exit cleanly (althouh you get a HUP
            ...
            if(command == "START")
            {
                ...
                if(g_child_pid != 0)
                {
                    ...
                    for(size_t idx(0); idx < c.size(); ++idx)
                    {
                        ...
                        {
                            // Calling this shutdown() I simulate the normal
                            // BIO_free_all() call... and it breaks the socket
                            //shutdown(c[idx]->get_socket(), SHUT_RDWR);
