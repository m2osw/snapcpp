 ...
 */
template<>
void convertData<controlled_vars::zbool_t>(controlled_vars::zbool_t& field, const QString& data)
{
    field = data.toInt() != 0;
}
} // namespace QtSerialization
...
void website_variable::read(QtSerialization::QReader& r)
{
    ...
    QtSerialization::QFieldBasicType<controlled_vars::zbool_t> required(comp, "website_variable::required", f_required);
    r.read(comp);
}
 ...
 * \return true if the parser succeeds, false otherwise.
 */
bool snap_uri_rules::parse_website_rules(const QString& script, QByteArray& result)
{
    // LEXER
