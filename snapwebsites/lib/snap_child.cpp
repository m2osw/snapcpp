...
// (much easier to debug a crashing problem!)
	...
	canonalize_domain();	// using the URI, find the domain core::rules and start the canonalization process
	canonalize_website();	// using the canonalized domain, find the website core::rules and continue the canonalization process
 ...
 *
 * \note
 * Note that the child is created for Cassandra, the plugins, the
 ...
 *
 * The canonalized domain is a domain name with sub-domains that
 ...
 */
void snap_child::canonalize_domain()
{
		...
		if(regex.exactMatch(sub_domains)) {
			...
			for(int v = 0; v < vmax; ++v) {
				...
				if(var->get_required()) {
					...
					// these make up the final canonalized domain name
					canonalized += snap_uri::urlencode(sub_domain_value, ".");
				}
			...
			if(canonalized.isEmpty()) {
				f_website_key = f_domain_key;
			}
			else {
				f_website_key = canonalized + "." + f_domain_key;
			}
			return;
		}
	}
 ...
 * process: canonalize_domain(). This is not done again in the websites.
 *
 ...
 * can write tools that show the results of this parser.
 */
void snap_child::canonalize_website()
{
	...
	for(int i = 0; i < max; ++i) {
				...
				for(int v = 0; v < vmax; ++v) {
					...
					if(var->get_part() == website_variable::WEBSITE_VARIABLE_PART_PATH) {
						...
						if(var->get_required()) {
							...
							// these make up the final canonalized domain name
							canonalized_path += "/" + snap_uri::urlencode(path_value, "~");
						}
		...
		if(matching) {
			...
			canonalized += protocol + "://" + f_website_key;
			f_uri.set_option("port", port);
			if(port.toInt() != 80) {
				canonalized += ":" + port;
			}
			if(canonalized_path.isEmpty()) {
				canonalized += "/";
			}
			else {
				canonalized += canonalized_path;
			}
			QString canonalized_query;
			for(QMap<QString, QString>::const_iterator it(query.begin()); it != query.end(); ++it) {
				f_uri.set_query_option(it.key(), it.value());
				if(!canonalized_query.isEmpty()) {
					canonalized_query += "&";
				}
				canonalized_query += snap_uri::urlencode(it.key()) + "=" + snap_uri::urlencode(it.value());
			}
			if(!canonalized_query.isEmpty()) {
				canonalized += "?" + canonalized_query;
			}
			// now we've got the site key
			f_site_key = canonalized;
 ...
 * include regular expressions are not getting canonalized
 ...
 *
 * The function is really fast if the path is already canonalized.
 *
 * \note
 ...
 *
 * \param[in,out] path  The path to canonalize.
 */
void snap_child::canonalize_path(QString& path)
{
