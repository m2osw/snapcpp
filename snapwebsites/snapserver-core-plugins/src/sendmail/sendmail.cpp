...
void sendmail::check_bounced_emails()
{
    ...
    //
    libdbproxy::table::pointer_t emails_table(get_emails_table());
    libdbproxy::row::pointer_t raw_row(emails_table->row(get_name(name_t::SNAP_NAME_SENDMAIL_BOUNCED_RAW)));
    raw_row->clearCache();
    ...
    for(;;)
    {
        ...
        libdbproxy::QCassandraCells const cells(raw_row->cells());
        if(cells.isEmpty())
        {
            break;
        }
    ...
    libdbproxy::row::pointer_t row(emails_table->row(get_name(name_t::SNAP_NAME_SENDMAIL_BOUNCED)));
    row->clearCache();
...
void sendmail::process_emails()
{
    ...
    libdbproxy::table::pointer_t emails_table(get_emails_table());
    libdbproxy::row::pointer_t row(emails_table->row(get_name(name_t::SNAP_NAME_SENDMAIL_NEW)));
    row->clearCache();
    auto column_predicate = std::make_shared<libdbproxy::cell_range_predicate>();
    column_predicate->setCount(100); // should this be a parameter?
    column_predicate->setIndex(); // behave like an index
...
void sendmail::attach_email(email const & e)
{
    ...
    libdbproxy::table::pointer_t emails_table(get_emails_table());
    libdbproxy::row::pointer_t lists(emails_table->row(get_name(name_t::SNAP_NAME_SENDMAIL_LISTS)));
    ...
    while(list.next(m))
    {
        ...
        if(!m.f_email_only.empty())
        {
            ...
            if(lists->exists(list_key))
            {
                ...
                is_list = true;
                libdbproxy::value list_value(lists->cell(list_key)->value());
...
SNAP_LOG_TRACE  ("sendmail::attach_user_email(): email=")(m.f_email_only)
...
#endif
    libdbproxy::table::pointer_t emails_table(get_emails_table());
    libdbproxy::row::pointer_t row(emails_table->row(user_key)); // TODO: convert to using user identifier
    if( !user_info.exists() )
    {
    ...
    //
    libdbproxy::value freq_value(row->cell(get_name(name_t::SNAP_NAME_SENDMAIL_FREQUENCY))->value());
...
void sendmail::run_emails()
{
    ...
    libdbproxy::row::pointer_t row(emails_table->row(index));
    row->clearCache();
    auto column_predicate = std::make_shared<libdbproxy::cell_range_predicate>();
...
void sendmail::sendemail(QString const & key, QString const & unique_key)
{
    ...
    libdbproxy::table::pointer_t emails_table(get_emails_table());
    libdbproxy::row::pointer_t row(emails_table->row(key));
    libdbproxy::value const sent_value(row->cell(sending_status)->value());
    ...
    libdbproxy::value email_data(row->cell(unique_key + "::" + get_name(name_t::SNAP_NAME_SENDMAIL_EMAIL))->value());
