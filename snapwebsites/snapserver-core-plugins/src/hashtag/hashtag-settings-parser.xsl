<?xml version="1.0"?>
<!--
Snap Websites Server == hashtag setting page data to XML
...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:variable name="layout-area">hashtag-settings-parser</xsl:variable>
	<xsl:variable name="layout-modified">2015-01-02 04:19:36</xsl:variable>
	<xsl:variable name="layout-editor">hashtag-settings-page</xsl:variable>

	<xsl:template match="snap">
		<output><!-- lang="{$lang}"-->
			<div id="content" class="editor-form" form_name="hashtag">
				...
				<h2>Hashtag Settings</h2>
				<div>
					<xsl:attribute name="class">test<!--xsl:if test="$action = 'edit'"> editing</xsl:if--></xsl:attribute>

					<fieldset>
						<legend>Manipulations</legend>

						<div class="editor-block">
							<xsl:copy-of select="page/body/hashtag/link/node()"/>
						</div>
					</fieldset>
