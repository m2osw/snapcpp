...
void path_info_t::force_extended_revision(QString const& revision, QString const& filename)
{
    ...
    {
        throw snap_logic_exception(QString("invalid version string (%1) in \"%2\" (force_extended_revision): not enough numbers (at least 2 required).").arg(revision).arg(filename));
    }
