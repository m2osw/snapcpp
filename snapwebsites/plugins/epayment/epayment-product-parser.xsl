	...
	<xsl:template match="snap">
		<output lang="{$lang}">
			<div id="content">
				...
				<div>
					<xsl:attribute name="class">epayment-product <xsl:value-of select="head/metadata/epayment/product-name/node()"/></xsl:attribute>
					<div>Product Name: <xsl:copy-of select="head/metadata/epayment/product-description/node()"/></div>
					<div class="epayment-button-area"><a href="buy-now" class="buy-now-button" product-guid="{$page_uri}">Buy Now</a></div>
					<div>Price: <xsl:copy-of select="head/metadata/epayment/product-price/node()"/></div>
				</div>
