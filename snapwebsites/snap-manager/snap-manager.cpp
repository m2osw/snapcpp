...
void snap_manager::cassandraConnect()
{
	...
	// also check for the 3 main tables
	snap::name_t names[3] = { snap::SNAP_NAME_DOMAINS, snap::SNAP_NAME_WEBSITES, snap::SNAP_NAME_SITES };
	for(int i = 0; i < 3; ++i) {
...
void snap_manager::on_domainDelete_clicked()
{
	...
	QString table_name(snap::get_name(snap::SNAP_NAME_DOMAINS));
	QSharedPointer<QtCassandra::QCassandraTable> table(f_context->findTable(table_name));
	table->dropRow(f_domain_name->text());
...
void snap_manager::on_websiteDelete_clicked()
{
	...
	QString table_name(snap::get_name(snap::SNAP_NAME_DOMAINS));
	QSharedPointer<QtCassandra::QCassandraTable> table(f_context->findTable(table_name));
	QString domain_name(f_domain_name->text());
	QSharedPointer<QtCassandra::QCassandraRow> row(table->findRow(domain_name));
	row->dropCell(f_website_name->text());
