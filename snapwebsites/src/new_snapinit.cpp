...
void service::configure(QDomElement e, QString const & binary_path, bool const debug)
{
    ...
    //
    {
        ...
        if(!sub_element.isNull())
        {
            ...
            tcp_client_server::get_addr_port(connect_addr_port, f_tcp_addr, f_tcp_port, 0);
            if(f_tcp_port <= 0)
            {
                SNAP_LOG_FATAL("Port for service \"")(f_service_name)("\" cannot be 0 or undefined.");
                exit(1);
                snap::NOTREACHED();
            }
        }
...
void service::process_timeout()
{
    ...
    //
std::cerr << "process_timeout() for \"" << f_service_name << "\" stopping: " << f_stopping << "\n";
    if(f_stopping != 0)
    {
        if(is_running())
        {
...
std::cerr << "connection? " << (is_connection_required() ? "YES" : "no") << "\n";
    if(is_connection_required())
    {
        ...
        else
        {
            ...
            //
std::cerr << "attempt starting service? " << f_service_name << "\n";
            snap::NOTUSED( run() );
        }
    }
    else if(is_running())
...
std::cerr << "pid already zero? " << f_service_name << " = " << f_pid << "\n";
...
if(f_failed) std::cerr << "*** PROCESS FAILED!\n";
        }
        else
        {
            f_short_run_count = 0;
    ...
    if( failed() )
    {
        ...
        if( recovery <= 0 )
        {
            ...
            //
std::cerr << "*** FAILED WITHOUT RECOVERY!\n";
            set_enable(false);
...
void snap_init::init()
{
    ...
    //
    {
        QString direct_listen("127.0.0.1:4039");
        if(f_config.contains("direct_listen"))
        {
            // use .conf definition when available
            direct_listen = f_config["direct_listen"];
        }
        tcp_client_server::get_addr_port(direct_listen, f_udp_addr, f_udp_port, 4039);
    }
...
void snap_init::remove_terminated_services()
{
    ...
    //
    {
std::cerr << "removing terminated services... " << f_service_list.size() << "\n";
...
std::cerr << "  -- count " << f_service_list.size() << "...\n";
        if(f_service_list.empty())
        {
